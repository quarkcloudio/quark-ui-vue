...
export const basicRouteMap = {
  // iframe模式下使用
  Iframe: () => import('~/components/Layout/Iframe.vue'),
  // 一般用于存在子集的页面
  RouteView: () => import('~/components/Layout/RouteView.vue'),
  // 空页面
  ComponentError: () => import('~/components/Exception/ComponentError.vue'),
}
